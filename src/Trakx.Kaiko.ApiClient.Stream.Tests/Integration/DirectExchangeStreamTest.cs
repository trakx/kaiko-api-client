...
{
    public class DirectExchangeStreamTest : KaikoClientStreamTestsBase
    {        
        private readonly KaikoDirectExchangeStreamHandler _streamClient;

        public DirectExchangeStreamTest(ITestOutputHelper output)
        {
            _streamClient = new KaikoDirectExchangeStreamHandler(_config, 
        ...
        public void test_multiple_pairs_stream()
        {
            ...
            foreach (var pair in _pairs)
            {
                Task.Run(async () =>
                {
                    ...
                    var _streamClientPerPair = new KaikoDirectExchangeStreamHandler(_config,
                        new StreamRequestParameter(pair, StreamRequestParameterAggregate.OneMinute),
                        handlerName);
